(function () {
  ...
  m.softmaxVec = function (vec) {
    ...
    var preSoftmaxVec = m.activateVec(vec, function (x) {
      return Math.exp(x - max);
    })
    return m.activateVec(preSoftmaxVec, function (x) {
      return x / m.sumVec(preSoftmaxVec)
    })
  };
  ...
  m.meanVecs = function (vecs) {
    ...
    return m.activateVec(sum, function (x) {
      return 1. * x / vecs.length;
    });
  };
  ...
  m.covarianceVecs = function (vecs) {
    ...
    return m.activateMat(mat, function (x) {
      return 1. * x / (vecs.length - 1);
    });
  };
